...
### 4.12 Расширяемая логика опросников
- Для сложных опросников должна поддерживаться обработка результатов по ключам/правилам (не только прямой расчет суммы по шкале).
- У каждого опросника может быть собственная процедура обработки и интерпретации результатов.
- Правила обработки должны задаваться декларативно в схеме опросника и иметь версионность.
- Исполнение правил должно быть безопасным: запрещено выполнение произвольного JS/вредоносного кода на устройстве пользователя.
- Для вычислений должен использоваться легковесный sandbox-подход (DSL/интерпретатор), пригодный для mobile/PWA.

### 4.13 Контент опросника и мультимедиа
- Опросник должен поддерживать вводный блок (`intro`) с пояснением целей и контекста.
- Опросник должен поддерживать завершающий блок (`outro`) с выводами (статический текст и/или текст на основе вычисленных параметров).
- Опросник должен поддерживать иконку/обложку (изображение уровня опросника).
- Вопрос должен поддерживать сопроводительное изображение.
- Опросник может быть помечен как регулярный и иметь расписание напоминаний.
- Для регулярных опросников должны поддерживаться PWA-напоминания (с явным согласием пользователя).
- Ответы пользователя должны поддерживать аудио и видео вложения (в дополнение к тексту и изображениям).
- Импорт/экспорт должен сохранять полный payload ответов с мультимедиа.
- Формат обмена по умолчанию остается JSON; для media-heavy кейсов должен поддерживаться опциональный двоичный пакетный формат с обратной совместимостью.

### 4.14 Пользовательская документация
- В разделе документации должна поддерживаться пользовательская инструкция по работе с приложением.
- Инструкция должна содержать отдельные сценарии для `desktop` и `mobile`.
- Инструкция должна быть разделена по профилям (`student`, `curator`, `admin`).
- Тип инструкции должен определяться по типу экрана (ширине viewport) и роли пользователя.
- Инструкция должна регулярно обновляться при изменениях UI.
- Обновление скриншотов инструкции должно выполняться через skill `playwright` в режиме `playwright-cli`.
- Канонический baseline пользовательских потоков для регрессионного тестирования ведется в `docs/testing/user-flow-baseline.md`.
- Отдельная пользовательская инструкция для конечных пользователей ведется в `docs/guides/user-manual.md`.
- Для каждого потока в инструкции должен быть стабильный `Flow ID`, шаги, ожидаемый результат и минимальные тестовые чекпоинты.
- Для каждой UI-задачи в `memory-bank/tasks/.../qa.md` должен фиксироваться список затронутых `Flow ID` и статус проверки (`pass/fail`).
- Должен существовать отдельный сценарный пакет `playwright-cli` (машиночитаемый), который проходит ключевые `Flow ID` и генерирует screenshot/assert-артефакты для UI/UX regression.

### 4.15 Иерархия папок и маршрутизация работ
- Для студента должна поддерживаться иерархия папок опросников (папки могут быть вложены друг в друга).
- Студент может произвольно перемещать опросники и папки в своей структуре.
- Для куратора должна поддерживаться иерархия папок работ студентов (группы/подгруппы).
- Для папки куратора должны отображаться агрегаты: количество студентов и количество свежих непроверенных работ.
- После первичной привязки студента к папке новые загружаемые работы этого студента должны автоматически попадать в ту же папку.
- Если при загрузке данных студента папка не определена, а папки уже существуют, система должна предложить выбрать существующую папку или создать новую.
...
- `UR-083`: Схема опросника поддерживает декларативные ключи/правила обработки результатов для сложных тестов (перцептивные каналы, психотипы, энеаграмма и др.).
- `UR-084`: Для каждого опросника допускаются индивидуальные правила вычисления и интерпретации результата, версионируемые в схеме.
- `UR-085`: Движок вычисления результатов работает в безопасной sandbox-среде (без выполнения произвольного JS и без возможности внедрения вредоносного кода).
- `UR-086`: Опросник поддерживает мультиязычные блоки `intro` и `outro`; `outro` может использовать вычисленные параметры результата.
- `UR-087`: Схема опросника поддерживает обложку/иконку опросника и сопроводительные изображения на уровне вопроса.
- `UR-088`: Опросник может быть помечен как регулярный и иметь расписание напоминаний.
- `UR-089`: Для регулярных опросников поддерживаются PWA-напоминания с управлением согласием пользователя.
- `UR-090`: Ответы пользователя поддерживают аудио и видео вложения наравне с текстом/изображениями.
- `UR-091`: JSON-импорт/экспорт сохраняет расширенные ответы и мультимедиа-метаданные без потери данных.
- `UR-092`: Для больших медиа-наборов поддерживается опциональный двоичный пакет обмена с обратной совместимостью по JSON.
- `UR-093`: В документации проекта поддерживается пользовательская инструкция с отдельными разделами для `desktop` и `mobile`.
- `UR-094`: Пользовательская инструкция разделена по профилям `student`, `curator`, `admin`.
- `UR-095`: Выбор релевантной версии инструкции определяется типом экрана (viewport) и текущим профилем пользователя.
- `UR-096`: Инструкция и ее скриншоты обновляются регулярно при UI-изменениях через `playwright-cli`.
- `UR-097`: Инструкция является каноническим baseline пользовательских flow для регрессионного тестирования новых фич.
- `UR-098`: Для каждого flow в инструкции фиксируются `Flow ID`, ожидаемый результат и тестовые чекпоинты; в QA UI-задач указываются затронутые `Flow ID` и статус проверки.
- `UR-099`: Поддерживается отдельный сценарный пакет `playwright-cli`, связанный с `Flow ID`, для регулярного UI/UX regression и генерации screenshot/assert-артефактов.
- `UR-100`: Для студента поддерживается иерархия папок опросников с вложенностью.
- `UR-101`: Студент может произвольно перемещать опросники и папки внутри своей структуры.
- `UR-102`: Для куратора поддерживается иерархия папок работ студентов.
- `UR-103`: Для каждой папки куратора показываются агрегаты: число студентов и число свежих непроверенных работ.
- `UR-104`: После первичной привязки студента к папке новые работы этого студента автоматически маршрутизируются в ту же папку.
- `UR-105`: При импорте неизвестного студента система предлагает выбрать существующую папку или создать новую.
- `UR-106`: Поддерживается ручная переупаковка работ студентов между папками куратором без потери истории и статусов проверки.
- `UR-107`: В правилах опросника поддерживаются механизмы проверки честности/валидности ответов (например, шкала лжи) с фиксацией итогового статуса в результате.
